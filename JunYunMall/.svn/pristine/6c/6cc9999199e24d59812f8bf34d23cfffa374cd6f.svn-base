...
@ContentView(R.layout.fragment_myorder_wait_comment_layout)
public class MyOrderWaitCommentFragment extends BaseFragment {

	/**
	 * 我的订单-->待评价
	 */

	/*@ViewInject(R.id.lv_myorder_wait_comment)
	private ListView mWaitCommentLv; //待评价ListView
	private MyOrderAllAdapter myOrderAllAdapter;
	private ArrayList<MyOrderAllEntity> orderWaitCommentEntities = new ArrayList<MyOrderAllEntity>();//外部listview
	...
	protected void fillView() {
		...
		for (int i = 0; i < 2; i++) {
			...
			orderWaitCommentEntities.add(new MyOrderAllEntity("莱薇儿精品女鞋", "等待评价", myOrderAllInnerEntities));
	...
	protected void loadData() {
		...
		if(myOrderAllAdapter == null){
			myOrderAllAdapter = new MyOrderAllAdapter(
					getActivity(), 
					orderWaitCommentEntities, 
					MyOrderAllEntity.MYORDER_FLAG_WAIT_COMMENT);
			mWaitCommentLv.setAdapter(myOrderAllAdapter);
		} else {
