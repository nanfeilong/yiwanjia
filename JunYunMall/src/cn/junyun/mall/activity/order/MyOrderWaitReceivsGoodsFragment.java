...
@ContentView(R.layout.fragment_myorder_wait_receivsgoods_layout)
public class MyOrderWaitReceivsGoodsFragment extends BaseFragment {
	/**
	 * 我的订单-->待收货
	 */
	@ViewInject(R.id.lv_myorder_wait_receivsgoods)
	private ListView mWaitReceivsGoodsLv; //待收货ListView
	private MyOrderAllAdapter myOrderAllAdapter;
	private ArrayList<MyOrderAllEntity> orderWaitReceivsGoodsEntities = new ArrayList<MyOrderAllEntity>();//外部listview
	...
	protected void fillView() {
		...
		for (int i = 0; i < 2; i++) {
			...
			orderWaitReceivsGoodsEntities.add(new MyOrderAllEntity("莱薇儿精品女鞋", "等待收货", myOrderAllInnerEntities));
		}
	...
	protected void loadData() {
		...
		if(myOrderAllAdapter == null){
			myOrderAllAdapter = new MyOrderAllAdapter(getActivity(), 
					orderWaitReceivsGoodsEntities, 
					MyOrderAllEntity.MYORDER_FLAG_WAIT_RECEIVS_GOODS) {
			...
			mWaitReceivsGoodsLv.setAdapter(myOrderAllAdapter);
		}else {
